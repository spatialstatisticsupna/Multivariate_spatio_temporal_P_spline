########################################################################
## Fits several multivariate Bayesian spatio-temporal P-spline models ##
########################################################################
mps_st <- function(carto=NULL,
                   data=NULL,
                   ID.area=NULL,
                   ID.year=NULL,
                   crimes=NULL,
                   Expcrimes=NULL,
                   prior.spatial=1,      ## Selected prior distributions for the coefficients of the  spatial P-splines (1: RW1 ; 2: RW2)
                   prior.temporal=1,     ## Selected prior distributions for the coefficients of the  temporal P-splines (1: RW1 ; 2: RW2)
                   prior.interaction=2,  ## Selected prior distributions for the espatio-temporal random effects (1: Type I; 2: Type II; 3: Type III; 4: Type IV)
                   temp.corre=TRUE,
                   order.B=3,            ## order B-splines
                   k.long=NULL,          ## Number of internal intervals (longitude)
                   k.lati=NULL,          ## Number of internal intervals (latitude)
                   k.time=NULL,          ## Number of internal intervals (time)
                   centered=TRUE,        ## TRUE: to center the smooth functions; FALSE: to center the coefficients
                   strategy="simplified.laplace",
                   initial=TRUE,
                   rerun=FALSE
                   ){
  
  ## Check for errors
  ## ----------------------------------------
  if(is.null(carto))
    stop("the carto argument is missing")
  if(!any(class(carto) %in% c("SpatialPolygonsDataFrame","sf")))
    stop("the carto argument must be of class 'SpatialPolygonsDataFrame' or 'sf'")
  if(is.null(data))
    stop("the data argument is missing")
  if(is.null(ID.area))
    stop("the ID.area argument is missing")
  if(is.null(crimes))
    stop("the crimes argument is missing")
  if(is.null(Expcrimes))
    stop("the Expcrimes argument is missing")
  
  if(!(prior.spatial %in% c(1:2)))
    stop("invalid prior.spatial argument")
  if(!(prior.temporal %in% c(1:2)))
    stop("invalid prior.temporal argument")
  if(!(prior.interaction %in% c(1:4)))
    stop("invalid prior.interaction argument")
  
  if(!(strategy %in% c("gaussian","simplified.laplace","laplace","adaptative")))
    stop("invalid strategy argument")

  
  ## ----------------------------------------
  ## Pre-processing data
  ## ----------------------------------------
  cat("STEP 1: Pre-processing data\n")
  
  ## Number of areas and number of time periods
  n <- length(unique(data[,ID.area]))
  t <- length(unique(data[,ID.year]))
  
  ## Parameter of the model (number of crimes)
  k <- length(crimes)
  
  ## Row-wisw Kronecker product
  Rten <- function(X1,X2){
    one1 <- matrix(1,1,ncol(X1))
    one2 <- matrix(1,1,ncol(X2))
    kronecker(X1,one2)*kronecker(one1,X2)
  }
  
  
  ## Construct the spatial B-spline basis (Bs) 
  ## ----------------------------------------
  
  ## Longitudes scale into the [0,1] interval
  x1 <- sp::coordinates(carto)[,1]
  x1 <- (x1-min(x1))/(max(x1)-min(x1))
  if(is.null(k.long)){ndx1 <- ceiling(min(length(unique(x1))/4, 40))} else {ndx1 <- k.long}
  dis1 <- (max(x1)-min(x1))/ndx1
  x1l <- min(x1)-dis1*0.05
  x1r <- max(x1)+dis1*0.05
  dx1 <- (x1r-x1l)/ndx1
  knots1 <- seq(x1l-order.B*dx1, x1r+order.B*dx1, by=dx1)
  
  ## The horizontal B-spline basis
  B1 <- splines::spline.des(knots1,x1,order.B+1, 0*x1)$design
  k1 <- dim(B1)[2]
  
  ## Latitudes scale into the [0,1] interval
  x2 <- sp::coordinates(carto)[,2]
  x2 <- (x2-min(x2))/(max(x2)-min(x2))
  if(is.null(k.lati)){ndx2 <- ceiling(min(length(unique(x2))/4, 40))} else {ndx2 <- k.lati}
  ndx2 <- ceiling(min(length(unique(x2))/4, 40))
  dis2 <- (max(x2)-min(x2))/ndx2
  x2l <- min(x2)-dis2*0.05
  x2r <- max(x2)+dis2*0.05
  dx2 <- (x2r-x2l)/ndx2
  knots2 <- seq(x2l-order.B*dx2, x2r+order.B*dx2, by=dx2)
  
  ## The vertical B-spline basis
  B2 <- splines::spline.des(knots2,x2,order.B+1, 0*x2)$design
  k2 <- dim(B2)[2]
  
  ## The spatial B-spline basis
  Bs <- Rten(B2,B1)
  ks<- dim(Bs)[2]
  
  
  ## Construct the temporal B-spline basis (Bt) 
  ## ----------------------------------------
  ## Times scale into the [0,1] interval
  xt <- unique(data[,ID.year])
  xt <- (xt-min(xt))/(max(xt)-min(xt))
  if(is.null(k.time)){ndxt <- ceiling(min(length(unique(xt))/4, 40))} else {ndxt <- k.time}
  dis.t <- (max(xt)-min(xt))/ndxt
  xtl <- min(xt)- dis.t*0.05
  xtr <- max(xt)+ dis.t*0.05
  dxt <- (xtr-xtl)/ndxt
  knots.t <- seq(xtl- order.B*dxt, xtr+order.B*dxt, by=dxt)
  
  ## The temporal B-spline basis
  Bt <- splines::spline.des(knots.t,xt,order.B+1, 0*xt)$design
  kt <- dim(Bt)[2]
  
  
  
  ## Adjacency and precision matrices 
  ## ----------------------------------------
  
  ## W1 and W2: adjacency matrices (spatial)
  P1 <- crossprod(diff(diag(k1), differences=prior.spatial))
  P2 <- crossprod(diff(diag(k2), differences=prior.spatial))
  
  ## I.P1 = I_{k2} (x) P1 and P2.I = P2 (x) I_{k1}
  I.P1 <- kronecker(Matrix::Diagonal(nrow(P2),1), P1)
  P2.I <- kronecker(P2, Matrix::Diagonal(nrow(P1),1))
  
  W1 <- INLA::inla.as.sparse(Matrix::Diagonal(n=nrow(I.P1),diag(I.P1))- I.P1)
  W2 <- INLA::inla.as.sparse(Matrix::Diagonal(n=nrow(P2.I),diag(P2.I))- P2.I)
  
  
  ## Wt: adjacency matrix (temporal)
  Pt <- crossprod(diff(diag(kt),differences=prior.temporal))
  
  Wt <- INLA::inla.as.sparse(Matrix::Diagonal(n=nrow(Pt),diag(Pt))- Pt) ## temp.corre = TRUE
  Cmat.t <- list(inla.as.sparse(Pt))                                    ## temp.corre = FALSE
  
  
  ## R.st: Precision matrices for spatio-temporal interaction
  spdep::nb2INLA("carto_nb.graph", spdep::poly2nb(carto))
  g <- INLA::inla.read.graph("carto_nb.graph")
  Qs <- matrix(0, g$n, g$n)
  for(i in 1:g$n){
    Qs[i,i]=g$nnbs[[i]]
    Qs[i,g$nbs[[i]]]=-1
  }
  Qt <- crossprod(diff(diag(t), differences=prior.temporal))
  
  
  if(prior.interaction %in% c(1)){
    R.st <- diag(n*t)
    r.def.st <- 0
  }
  if(prior.interaction %in% c(2)){
    R.st <- kronecker(Qt,diag(n))
    r.def.st <- prior.temporal*n
  }
  if(prior.interaction %in% c(3)){
    R.st <- kronecker(diag(t),Qs)
    r.def.st <- t
  }
  if(prior.interaction %in% c(4)){
    R.st <- kronecker(Qt, Qs)
    r.def.st <- prior.temporal*n+t-1
  }
  
  ## Define appropriate constraints matrices (A.constr)
  ## ----------------------------------------
  if(centered){ 
    A.constr.s <- kronecker(diag(1,k), matrix(apply(Bs,2,sum), nrow=1, ncol=ks))
    A.constr.t <- kronecker(diag(1,k), matrix(apply(Bt,2,sum), nrow=1, ncol=kt))
  } else {
    A.constr.s <- kronecker(diag(1,k), matrix(1, nrow=1, ncol=ks))
    A.constr.t <- kronecker(diag(1,k), matrix(1, nrow=1, ncol=kt))
  }
  
  
  if(prior.interaction %in% c(1)){
    A.constr.st <- matrix(1, nrow=1, ncol=n*t)
  }
  if(prior.interaction %in% c(2)){
    A.constr.st <- kronecker(matrix(1,nrow=1, ncol=t), diag(1,n))
    if(prior.spatial==2 | prior.temporal==2){
      A.constr.st <- rbind(A.constr.st, kronecker(matrix(1:t,nrow=1, ncol=t), matrix(c(1, rep(0,n-1)),nrow=1, ncol=n)))
    }
  }
  if(prior.interaction %in% c(3)){
    A.constr.st <- kronecker(diag(1,t), matrix(1,nrow=1, ncol=n))
  }
  if(prior.interaction %in% c(4)){
    A1 <- kronecker(matrix(1,nrow=1, ncol=t), diag(1,n))
    if(prior.spatial==2 | prior.temporal==2){
      A1 <- rbind(A1, kronecker(matrix(1:t,nrow=1, ncol=t), matrix(c(1, rep(0,n-1)),nrow=1, ncol=n)))
    }
    A2 <- kronecker(diag(1,t), matrix(1,nrow=1, ncol=n))
    A.constr.st <- rbind(A1[-1,], A2)
  }
  
  ## Design matrices for random effects (D.matrix)
  ## ----------------------------------------
  
  ## D.inter.j
  D.inter<- kronecker(diag(1, nrow = k), matrix(1, nrow = n*t, ncol=1))
  eval(parse(text = paste0("D.inter.",1:k,"<- matrix(D.inter[,",1:k,"], ncol=1)")))
  
  ## D.spat = I_{J} (x) 1_{t} (x) Bs
  D.spat<- kronecker(diag(1, nrow = k), kronecker(matrix(1, nrow = t, ncol=1), Bs))
  
  ## D.temp = I_{J} (x) Bt (x) 1_{n}
  D.temp<- kronecker(diag(1, nrow = k), kronecker(Bt, matrix(1, nrow=n, ncol=1))) ## temp.corre = TRUE
  for(i in 1:k){
    eval(parse(text = paste0("D.temp.",i,"<- D.temp[,(",i,"-1)*kt + 1:kt]")))     ## temp.corre = FALSE  
  }
  
  ## D.spat.temp.j
  for(i in 1:k){
    aux<- matrix(NA, t*n*k, t*n)
    aux.block<- suppressWarnings(blockmatrix::as.blockmatrix(aux, nrowe=t*n, ncole=t*n, nrow=k, ncol=1))
    aux.block[[i]]<- diag(1, t*n)
    aux<- as.matrix(aux.block)
    eval(parse(text = paste0("D.spat.temp.",i,"<-aux") ))
    rm(list = c("aux", "aux.block"))
  }
  
  
  ## Design matrix
  if(!temp.corre){
    D.matrix <- cbind(D.inter.1,D.inter.2,D.inter.3,D.inter.4,
                      D.spat, D.temp.1,D.temp.2,D.temp.3,D.temp.4,
                      D.spat.temp.1, D.spat.temp.2, D.spat.temp.3, D.spat.temp.4)
  } else {
    D.matrix <- cbind(D.inter.1, D.inter.2, D.inter.3, D.inter.4,
                      D.spat, D.temp,
                      D.spat.temp.1, D.spat.temp.2, D.spat.temp.3, D.spat.temp.4)
  }
  
  
  
  ## Data for INLA 
  ## ----------------------------------------
  data<- data[order(data[,ID.year], data[,ID.area]),]
  obs<- reshape2::melt(data, id.vars = c(ID.area,ID.year), measure.vars = crimes,
                       variable.name = "crime",
                       value.name = "obs")
  exp<- reshape2::melt(data, id.vars = c(ID.area,ID.year), measure.vars = Expcrimes,
                       variable.name = "crime",
                       value.name = "exp")
  
  df.inla<-data.frame(O=obs[,"obs"],
                      E=exp[,"exp"],
                      crime=obs[,"crime"],
                      ID_area= obs[,ID.area],
                      ID_year= obs[,ID.year],
                      ID_crime=rep(1:k,each=n*t))
  
  ## 'id.idx'
  id.idx<- c(rep(NA,k), 1:(ks*k), rep(NA, kt*k + n*t*k))
  
  ## 'id.idy'
  id.idy<- c(rep(NA,k + ks*k), 1:(kt*k), rep(NA, n*t*k))
  
  ## 'id.inter.j', 'id.idxy.j' and 'id.idy.j'
  for(i in 1:k){
    ## id.inter.j
    aux<- rep(NA, k*(1+ks+kt+n*t))
    aux[i]<-1
    eval(parse(text = paste0("id.inter.",i,"<-aux") ))
    rm(aux)
    
    ## id.idxy.j
    aux<- rep(NA, k*(1+ks+kt+n*t))
    aux[(k*(1+ks+kt)+(i-1)*(n*t))+1:(n*t)]<- 1:(n*t)
    eval(parse(text = paste0("id.idxy.",i,"<-aux") ))
    rm(aux)
    
    ## id.idy.j (temp.corre = FALSE)
    aux<- rep(NA,k + ks*k + kt*k + n*t*k)
    aux[(k*(1+ks)+(i-1)*kt) + 1:kt]<- 1:kt
    eval(parse(text = paste0("id.idy.",i,"<-aux") ))
    rm(aux)
  }
  
  ## Data.INLA
  
  if(!temp.corre){
    Data.INLA<- list(O = df.inla$O, E = df.inla$E, idx=id.idx)
    for (i in 1:k){
      eval(parse(text = paste0("Data.INLA$I",i,"<-id.inter.",i) ))
      eval(parse(text = paste0("Data.INLA$idy.",i,"<- id.idy.",i) ))
      eval(parse(text = paste0("Data.INLA$idxy.",i,"<- id.idxy.",i) ))
    }
  } else {
    Data.INLA<- list(O = df.inla$O, E = df.inla$E, idx=id.idx, idy=id.idy)
    for (i in 1:k){
      eval(parse(text = paste0("Data.INLA$I",i,"<-id.inter.",i) ))
      eval(parse(text = paste0("Data.INLA$idxy.",i,"<- id.idxy.",i) ))
    }
  }
  
  
  
  ## Initial values for hyperparameters
  ## ----------------------------------------
  
  ## Spatial initial values (hyperparameters)
  library(tidyverse)
  sir.s<- df.inla %>%
    group_by(ID_crime, ID_area) %>% 
    summarise(SIR = sum(O)/sum(E)) 
  Sigma.s <- cov(matrix(sir.s$SIR, nrow=n, ncol=k, byrow = FALSE))
  Rho.s <- cor(matrix(sir.s$SIR, nrow=n, ncol=k, byrow = FALSE))
  
  
  
  ## Temporal initial values (hyperparameters)
  sir.t <- df.inla %>%
    group_by(ID_crime, ID_year) %>%
    summarise(SIR = sum(O)/sum(E))
  Sigma.t <- cov(matrix(sir.t$SIR, nrow=t, ncol=k, byrow = FALSE))
  Rho.t <- cor(matrix(sir.t$SIR, nrow=t, ncol=k, byrow = FALSE))
  
  if(!initial){
    initial.values.s<- as.vector(c(rep(1,k), rep(1, k*(k-1)/2), rep(0.5,2)))
    initial.values.t<- as.vector(c(rep(1,k), rep(1,k*(k-1)/2), rep(0.5,1)))
  } else {
    initial.values.s<- as.vector(c(sqrt(diag(Sigma.s)), Rho.s[upper.tri(Rho.s, diag = FALSE)], rep(0.5,2)))
    initial.values.t<- as.vector(c(sqrt(diag(Sigma.t)), Rho.t[upper.tri(Rho.t, diag = FALSE)], rep(0.5,1)))
  }
  
  ## Define appropriate hyperprior distributions (spatio-temporal)
  ## ----------------------------------------
  sdunif="expression:
  logdens=-log_precision/2;
  return(logdens)"
  
  
  ## ----------------------------------------
  ## Fitting model
  ## ----------------------------------------
  
  cat("STEP 2: Fitting model with INLA (this may take a while...)\n")
  
  
  ## To compute patterns (as linear combinations of the log-risks)
  ## ----------------------------------------
  lc.spat <- INLA::inla.make.lincombs(idx = kronecker(diag(k),Bs))
  names(lc.spat) <- paste("spatial.",as.character(seq(1,k*n,1)),sep="")
  
  if(!temp.corre){
    lc.temp.1 <- INLA::inla.make.lincombs(idy.1 = Bt)
    names(lc.temp.1)<- paste("temporal.1.",as.character(seq(1,t,1)),sep="")
    lc.temp.2 <- INLA::inla.make.lincombs(idy.2 = Bt)
    names(lc.temp.2)<- paste("temporal.2.",as.character(seq(1,t,1)),sep="")
    lc.temp.3 <- INLA::inla.make.lincombs(idy.3 = Bt)
    names(lc.temp.3)<- paste("temporal.3.",as.character(seq(1,t,1)),sep="")
    lc.temp.4 <- INLA::inla.make.lincombs(idy.4 = Bt)
    names(lc.temp.4)<- paste("temporal.4.",as.character(seq(1,t,1)),sep="")
    
    all.lc <- c(lc.spat,lc.temp.1,lc.temp.2,lc.temp.3,lc.temp.4)
    
  }
  if(temp.corre){
    lc.temp <- INLA::inla.make.lincombs(idy = kronecker(diag(k),Bt))
    names(lc.temp) <- paste("temporal.",as.character(seq(1,k*t,1)),sep="")
    
    all.lc <- c(lc.spat,lc.temp)
  }
  
  
  
  ## Implementation of the spatio-temporal P-spline models
  ## ----------------------------------------
  # source("functions/generic_mps.R")
  model.s <- INLA::inla.rgeneric.define(generic_mps_spat, debug = FALSE, k = k, W1 = W1, W2 = W2,
                                        initial.values = initial.values.s)
  
  model.t <- INLA::inla.rgeneric.define(generic_mps_temp, debug = FALSE, k = k, W = Wt,
                                        initial.values = initial.values.t)
  
  ## Define the 'formula'
  form <- "O ~ -1 +"
  form <- paste(form, paste(paste0("I",1:k), collapse="+"), sep=" ")
  form <- paste(form, "+ f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A.constr.s, e=rep(0,dim(A.constr.s)[1]) ))", sep=" ")
  if(!temp.corre){
    form <- paste(form,"+",paste(paste0("f(idy.",1:k,", model = 'generic3', Cmatrix = Cmat.t, constr=TRUE, hyper=list(prec1=list(prior=sdunif)))"), collapse=" + "), sep=" ")
  }
  if(temp.corre){
    form <- paste(form, "+ f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A.constr.t, e=rep(0,dim(A.constr.t)[1]) ))", sep=" ")
  }
  form <- paste(form, "+", paste(paste0("f(idxy.",1:k,", model='generic0', Cmatrix=R.st, rankdef=r.def.st, constr=FALSE, extraconstr=list(A=A.constr.st, e=rep(0,dim(A.constr.st)[1])), hyper=list(prec=list(prior=sdunif)) )"), collapse=" + "), sep=" ")
  
  formula.inla <- stats::as.formula(form)
  
  ## Fit the INLA model
  Model<- INLA::inla(formula.inla, data = Data.INLA, E = E, family = "poisson",
                     control.predictor= list(compute = TRUE, A= D.matrix, link=1, cdf=c(log(1))),
                     control.compute=list(dic=TRUE, cpo=TRUE, waic=TRUE),
                     lincomb=all.lc,
                     control.inla = list(strategy=strategy),
                     debug = F,
                     verbose = F)
  if(rerun){ Model <- INLA::inla.rerun(Model) }
  
  Model$model <- list(prior.spatial=paste0("RW",prior.spatial),
                      prior.temporal=paste0("RW",prior.temporal),
                      prior.interaction=paste0("Type ",prior.interaction),
                      centered=centered,
                      temp.corre=temp.corre,
                      rerun=rerun
                      )
  
  return(Model)
}
########################################################################
########################################################################
...
'mult.spat.Psplines' <-
  function(cmd = c("graph", "Q", "mu", "initial", "log.norm.const",
    ...
    interpret.theta = function(){
      ...
      sigma.lambda<- exp(-(1/2)*theta[as.integer(k*(k+1)/2+1:2)])
    ...
    Q = function(){
      ...
      P<- (param$sigma.lambda[1]^(-2) ) * ( Matrix::Diagonal(nrow(W2), apply(W2, 1, sum)) - W2) +
        (param$sigma.lambda[2]^(-2) ) * ( Matrix::Diagonal(nrow(W1), apply(W1, 1, sum)) - W1)
    ...
    initial = function() {
      ...
      return ( c(rep(1,k), rep(1, k*(k-1)/2), rep(0.5,2)) )
    }
    ########################################################################
    ########################################################################
    quit = function() {

...
for(mm in 1:k){
  ...
  for(type in 1:4){
    ...
    formula.inla<- OBS ~ -1 + intercept +
      f(idx, model = "generic3", Cmatrix=Cmat.s, constr=FALSE, extraconstr=list(A=A.constr.s, e=e.s), 
        hyper=list(prec1=list(prior=sdunif), prec2=list(prior=sdunif)) ) +
      f(idy, model = "generic3", Cmatrix=Cmat.t, constr=FALSE, extraconstr=list(A=A.constr.t, e=e.t),
  ...
  ## save
  save(resulta, file = paste0("resul/resul_univ_Centered_Psplines_ps",ord.spat,"_pt",ord.temp,"_",crimes[[mm]],"_",gsub("\\.", "_", strategy),".RData"))

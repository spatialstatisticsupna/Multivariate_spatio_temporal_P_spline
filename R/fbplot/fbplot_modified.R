...
#method: BD2, BD3, MBD
fbplot=function(fit,x=NULL,method='MBD',depth=NULL,plot=TRUE,prob=0.5,color="indianred4",outliercol=2,
  ...
  if (length(depth)==0){
	...
	plot(x,fit[,index[1]],lty=1,lwd=2,col=1,type='l',...)
	  rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "gray93", border ="gray93")
	  abline(h=c(seq(0,4,1)),col="white")
	  abline(v=c(seq(2,12,2)),col="white")
	...
	for (pp in 1:length(prob)){
		...
		if (prob[pp]==0.5){ #check outliers
			...
			if (sum(outly)>0){
				if (plot) {
				matlines(x,out,lty=2,col=outliercol,lwd = 2,type='l',...)
				}
			}
			barval=(x[1]+x[tp])/2

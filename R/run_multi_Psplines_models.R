...
model<- inla(formula.inla, data = Data, E = EXP, family = "poisson",
                  control.predictor= list(compute = TRUE, 
                                          A= cBind(D.inter.1,D.inter.2,D.inter.3,D.inter.4, 
                                                   D.spat, D.temp,
                                                   D.spat.temp.1, D.spat.temp.2, D.spat.temp.3, D.spat.temp.4), link=1, cdf=c(log(1))),
...
## save
save(model, file= paste0("resul/resul_Psplines_ps",ord.spat,"_pt",ord.temp,"_type",type,"_",gsub("\\.", "_", strategy),".RData"))
